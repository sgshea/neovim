(module core.plugin
  {autoload {a aniseed.core
             packer packer}})
;; From the magic kit
;; https://github.com/Olical/core-kit

(defn- safe-require-plugin-config [name]
  "Safely require a module under the core.plugin.* prefix. Will catch errors
  and print them while continuing execution, allowing other plugins to load
  even if one configuration module is broken."
  (let [(ok? val-or-err) (pcall require (.. "core.plugin." name))]
    (when (not ok?)
